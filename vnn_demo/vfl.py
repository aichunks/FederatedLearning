...
class VFLHostModel(object):
    ...
    def load_weights(self,weights):
        self.localModel.restore_model(weights)
        self.W=weights["W_weight"]

class VFLGuestModel(PartyModelInterface):
...
class VerticalMultiplePartyFederatedLearning(BaseModel):
    ...
    def __init__(self, party_a: VFLHostModel, main_party_id="K", verbose=False):
    ...
    def add_party(self, *, id, party_model:VFLGuestModel):
    ...
    def saveModels(self,modelpath):
        if self.verbose: print("Saving model at ",modelpath)
        party_a_modelperemeter=self.party_a.localModel.get_model_parameters()
        party_a_modelperemeter["W_weight"]=self.party_a.W
        party_modelperemeter_list=[party.localModel.get_model_parameters() for party in self.party_dict.values()]
        for parameter,party in zip(party_modelperemeter_list,self.party_dict.values()):
            parameter["W_weight"] = party.W
        party_a_modelperemeter["W_weight"]=self.party_a.W
        with open(modelpath+".pk","wb") as f:
            pk.dump({"party_a_modelperemeter":party_a_modelperemeter,"party_modelperemeter_list":party_modelperemeter_list},f)
        if self.verbose: print("Model Saved at ",modelpath)
